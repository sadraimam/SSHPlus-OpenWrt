...
# SSHPlus Standalone Gateway - Native FW4 & Redsocks Architecture (V3)
# Built for OpenWrt 22.03.3
# Hardened: Explicit FW4 pipe-injection, WAF-resistant IP checking.
...
function api_handler()
	...
	if action == "status" then
		...
		if running then
			...
			-- Swapped to api.ipify.org to bypass Cloudflare/WAF 403 blocks
			local ip_handle = io.popen("curl --max-time 5 -s https://api.ipify.org")
...
# Wrap the nft syntax in a standard shell script so 'ash' feeds it to the nft binary correctly
cat > /tmp/sshplus_firewall.sh <<EOF
#!/bin/sh
/usr/sbin/nft -f - <<'NFT'
table inet fw4 {
...
NFT
EOF
chmod +x /tmp/sshplus_firewall.sh

uci -q delete firewall.sshplus_include
uci set firewall.sshplus_include=include
uci set firewall.sshplus_include.type='script'
uci set firewall.sshplus_include.path='/tmp/sshplus_firewall.sh'
# Intentionally removed the deprecated 'reload' option
uci commit firewall
